...
"""
    ...
    for f in faces:
        if( f != face ): #Se não for a face externa
            aux  = triangulaMonotono(f.listaFace())
            for bla in aux: diagonais2.append(bla)
    ...
    for diag in diagonais2:
        for k in range(len(pontos)):
            if(pontos[k][0].x == diag.getAresta().init[0].x and pontos[k][0].y == diag.getAresta().init[0].y): inicio = pontos[k]
            elif(pontos[k][0].x == diag.getAresta().to[0].x and diag.getAresta().to[0].y): fim = pontos[k]
        
        print inicio,fim
        d = criaAresta(inicio,fim)
        insereDiagonal(verticesDCEL[inicio[1]-1],verticesDCEL[fim[1]-1],d,faces2)
        diagonais.append(d)

    for diag in diagonais:
        insereDiagonal(verticesDCEL[diag.getAresta().init[1]-1],verticesDCEL[diag.getAresta().to[1]-1],diag,faces2)
