...
Primeiramente explicaremos um algoritmo que resolve este tipo de consultas no espaço $\mathbb{R}$, ou seja, os segmentos serão intervalos na
reta. Definiremos a ``janela'' como um dado ponto no espaço e encontraremos todos os segmentos que interceptam esse ponto.
...
\begin{lstlisting}[mathescape]
        while i < n and s[i].beg <= v.point:
            if s[i].end < v.point:
                ...
                l1.append(Point(s[i].beg.x,s[i].beg.y,s=s[i])
                l2.append(Point(s[i].end.x,s[i].end.y,s=s[i])
            i+=1
