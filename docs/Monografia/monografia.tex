...
algoritmo mais simples que estende a ideia apresentada no algoritmo anterior e no tópico seguinte uma estrutura de dado diferente
que pode ser usada neste algoritmo para diminuir o consumo de tempo.
...
o ponto $p$, um ponteiro para o menor ponto $q$ em $V^y_e$ tal que $q \geq_y p$, um ponteiro para o menor ponto $u$ em $V^y_d$ tal
que $u \geq_y p$, uma variável booleana que indica se o vetor ao qual o nó pertence é $V^y_e$ ou $V^y_d$ e finalmente um ponteiro
para o próximo elemento do vetor. Esse último ponteiro foi uma adaptação ao fato da linguagem \emph{python} não apresentar
aritmética de ponteiros, que foi utilizada na descrição desse algoritmo na dissertação de \citet{junio09:MSc}.
