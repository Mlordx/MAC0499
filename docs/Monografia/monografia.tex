...
seja $P = \{p_1,p_2,\dots,p_n\}$ tal que $p_1 \leq_{x} p_2 \leq_{x} \dots \leq_{x} p_n$, chamaremos $p_1$ de o \textbf{ x-menor} e $p_n$ de
o \textbf{x-maior} pontos de $P$. Todas essas definições são simétricas para a ordem $\leq_y$.
...
\begin{itemize}
\item[-] Consumimos tempo $\mathcal{O}(1)$ por nó visitado que está na resposta e só continuamos a fazer chamadas da função quando $p \leq w_1$. Portanto, chegamos à complexidade total de $\mathcal{O}(k)$.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%
% Melhorar essa seção, é O(k) mas precisa explicar melhor.
%%%%%%%%%%%%%%%%%%%%%%%
...
espaço, esses algoritmos serão posteriormente usados para resolver o problema proposto no próximo capítulo.
...
\begin{algorithm}[H]
\ContinuedFloat
\caption{Continuação do algoritmo 19.}
\lstset{basicstyle=\ttfamily\color{text}\bfseries}
\begin{lstlisting}[name=alg19]
        i = 0
...
\begin{algorithm}[H]
\ContinuedFloat
\caption{Continuação do algoritmo 22.}
\lstset{basicstyle=\ttfamily\color{text}\bfseries}
\begin{lstlisting}[name=alg22]
    if v is not None:
        if x > v.point.x:
...
\begin{itemize}
\item[-] Na construção, construimos listas com os pontos extremos dos segmentos consumindo tempo $\mathcal{O}(n)$ e realizamos
