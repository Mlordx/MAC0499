...
\in \mathbb{R}, $ definimos um \textbf{ponto} no $\mathbb{R}^2$ como um par $ p = (x,y) $. 
...
ordenados pela ordem $\leq_x$ (ou $\geq_{x}$) diremos que estes pontos estão ordenados \textbf{sobre a coordenada \emph{x}}. Além disso, seja $P =
\{p_1,p_2,\dots,p_n\}$ tal que $p_1 \leq_{x} p_2 \leq_{x} \dots \leq_{x} p_n$, chamaremos $p_1$ de o \textbf{ x-menor} e $p_n$ de o
\textbf{x-maior} pontos de $P$. Todas essas definições são simétricas para a ordem $\leq_y$.
...
def query_r(self,v,p):
    ...
    if v is not None:
        if p > v.point:
            ...
            aux = v.L2.maxheap_query(rng) |\label{alg20:11}|
            for pnt in aux:
                l.append(pnt.seg)
            ...
            aux = v.L1.minheap_query(rng) |\label{alg20:18}|
            for pnt in aux:
                l.append(pnt.seg)
