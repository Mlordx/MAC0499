...
$x,y \in \mathbb{R} $ definimos um \textbf{ponto} no $\mathbb{R}^2$ como um par $ p = (x,y) $. Um \textbf{segmento} $s$ é da forma
$ s \coloneqq \overline{(x_1,y_1)(x_2,y_2)}\in \mathbb{R}^2\times\mathbb{R}^2$ onde $u = (x_1,y_1)$ e $v = (x_2,y_2)$ são pontos
...
estrutura de dados apropriada. A estrutura que usaremos será um tipo de \emph{árvore de busca binária balanceada} ( ABBB ) chamada
de árvore limite, a seguir está o trecho de código referente à construção da árvore:
...
\begin{itemize}
\item[1.] Montamos a ABBB sobre o conjunto P.
\item[2.] Achamos o \emph{ponto divisor} de $P$, este é o ponto que se encontra na raiz da subárvore que contém os pontos
...
\begin{algorithm}
\caption{Retorna \emph{true} caso o ponto $p$ esteja contido no intervalo \emph{rng}.}
\lstset{basicstyle=\ttfamily\color{text}\bfseries}
\begin{lstlisting}[mathescape]
