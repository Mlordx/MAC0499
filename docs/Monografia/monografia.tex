...
\begin{itemize}
\item[-] Começamos achando o nó divisor dessa árvore, a estrutura básica é similar à implementada antes, porém agora checamos se o nó checado não é um
  semi-folha e também já adicionamos na resposta todos os \emph{pmin} dos nós acessados que estão dentro da janela na resposta final.
\item[-] Percorremos a subárvore esquerda de $v_{div}$ enquanto o nó atual não é uma folha ou semi-folha. Seja $v$ o nó que estamos verificando, se o
  ponto $pmin(v) <_y w_1$, caso seja, adicionamos todas os pontos do \emph{minheap} da subárvore direita de $v$ na resposta e seguimos para a
  subárvore esquerda de $v$, caso contrário apenas seguimos para a subárvore direita de $v$.
\item[-] Seja $u$ o último nó verificado, caso $pmin(u)$ esteja na resposta adicionamos esse ponto na resposta. Caso $u$ seja uma semi-folha, verificamos se o $pmin(u.l)$ está na janela e o adicionamos na resposta.
\item[-] Repetimos o processo simetricamente para a subárvore direita de $v_{div}$.
\end{itemize}

Segue os códigos que explicitam essa rotina:
